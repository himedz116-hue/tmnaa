...
'الموقع فيه أقسام مختلفة تقدر توجه المستخدم لها باستخدام هذه الأزرار:',
'- الرئيسية: [nav:الرئيسية:home]',
'- عن القناة: [nav:عن tmnaa:about]',
'- المجتمع: [nav:المجتمع:community]',
'- المعرض / الصور: [nav:المعرض:gallery]',
'- جدول البث: [nav:جدول البث:schedule]',
...
export default function ChatWidget() {
  ...
  const renderText = useCallback((text: string, sendFn: (text: string) => void) => {
    ...
    const parts: any[] = [];
    const re = /\[social:([a-zA-Z0-9_]+):([^:]*):(https?:\/\/[^\]]+)\]|\[emote:([^\]]+)\]|\[suggest:([^\]]+)\]|\[link:([^:]+):(https?:\/\/[^\]]+)\]|\[nav:([^:]+):([^\]]+)\]|(tmnaasalutetmnaa|tmnaatmnaalaughtmnaatmnaalaugh|tmnaascraptmnaaMTONTOPTMNAA)/g;
    let li = 0, m, k = 0;
      ...
      if (m[1]) {
        ...
        parts.push(
          <button 
            ...
            className="group flex items-center justify-between w-full mt-2 px-4 py-2.5 bg-gradient-to-l from-[#53FC18]/10 to-[#0a1a05] hover:from-[#53FC18]/20 border border-[#53FC18]/25 hover:border-[#53FC18]/50 rounded-xl text-[#b5f5a0] hover:text-white text-[13px] font-semibold transition-all duration-300 shadow-md hover:shadow-[0_0_20px_rgba(83,252,24,0.15)] cursor-pointer"
          >
            <span className="flex items-center gap-2 text-right">
              <span className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#53FC18]/15 text-[#53FC18] text-sm">📍</span>
              {navName}
            </span>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4 text-[#53FC18]/60 group-hover:text-[#53FC18] group-hover:-translate-y-0.5 transition-all"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
          </button>
        );
      }
      li = re.lastIndex;
