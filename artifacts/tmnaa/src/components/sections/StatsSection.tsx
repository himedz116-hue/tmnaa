...
export function StatsSection() {
  ...
  const [selectedVideo, setSelectedVideo] = useState<{ source?: string; uuid?: string | null; title?: string; session_title?: string; views?: number; view_count?: number } | null>(null);
          ...
          <div className="space-y-6">
            ...
            {videos ? videos.length > 0 ? (
              <div className="space-y-4">
                {videos.map((video) => {
                  return (
                    <button key={video.id} onClick={() => setSelectedVideo({ source: video.source, uuid: video.uuid, title: video.title, session_title: video.session_title, views: video.views, view_count: video.view_count })}
